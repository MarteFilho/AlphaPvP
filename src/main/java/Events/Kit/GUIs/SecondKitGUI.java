...
public class SecondKitGUI
{
    public static void create(AlphaPlayerPvP alphaPlayerPvP)
    {
        Inventory inventory = createInventory(alphaPlayerPvP, "Selecionar kit 2", 54);
        ...
        LinkedList<Kit> kits;
        if (alphaPlayerPvP.getRank().getName().equalsIgnoreCase("Membro"))
            kits = alphaPlayerPvP.getKitsPurchased();
        else
            kits = Kit.GetAllKits();

        for (Kit kit : kits)
        {
            if (alphaPlayerPvP.getSecondKit().GetName().equalsIgnoreCase(kit.GetName()))
            {
        ...
        inventory.setItem(49, ItemStackManager.Create("§eKit Selecionado: §a" + alphaPlayerPvP.getSecondKit().GetName(),
                Collections.singletonList(""), alphaPlayerPvP.getSecondKit().GetItemMenu(), 1));
